...
OpenTop 是给 AI 应用开发者用的选题雷达。它把趋势、GitHub issue、竞品链接或调研笔记，变成可比较、可发布的开源 AI 应用方案。

它主要回答三个问题：

- 值不值得做：按痛点、紧迫性、分发、可构建性和 Star 潜力给机会排序。
- 第一版做什么：给出目标用户、MVP 边界、验证步骤和主要风险。
...
中文体验是内置能力：浏览器语言为中文时默认进入简体中文界面；用户也可以在应用顶部切换 English / 简体中文。选择简体中文后，内置示例、本地 demo 输出和模型提示都会优先使用中文。

在线演示：[已验证的 rawcdn 备用 demo](https://rawcdn.githack.com/dhb118/opentop/c0cc556035b729caf53d44d9f0b4a1e5dc85adea/)。这是当前公开 demo，原生 GitHub Pages 恢复前使用这个固定的 `gh-pages` 构建。
...
![OpenTop 在线 demo 截图：内置简报、评分灯点和机会图库](docs/assets/opentop-live-demo.png)
...
- 示例验证：机会图库包含 16 个内置 AI 开发者简报，覆盖本地模型、Agent、MCP server、RAG、eval、Provider 迁移、发布风险和 README 定位。
...
- 仓库 metadata 应用：`GITHUB_TOKEN=github_pat_... pnpm repo:profile:apply` 或 `GH_TOKEN=github_pat_... pnpm repo:profile:apply` 可在发布前同步 About 描述、homepage、issues 开关和 topics。
...
- 记住 English / 简体中文界面偏好，提供中文默认简报、中文本地分析结果和中文模型提示。
- 导出 README 简报、公开发布简报、公开构建日志、90 秒 demo 脚本、Launch Kit、Star 增长计划、仓库 Profile 包、贡献者 issue 队列、Product Hunt 草稿和 newsletter pitch。
